...
- 参数是**字符串表达式**（与 `PRINTS` 相同，而非 `PRINT` 的纯字符串）
- 执行后自动换行（行为接近 `PRINTSL`）
- **EM+EE 扩展**：第二参数为非 0 时，不强制换行
...
```
<img src='资源名' srcb='选中时资源名' height='高%' width='宽%' ypos='纵向偏移%'>
```
...
**EM+EE 扩展**：
- `px` 表示法：数值后可加 `px` 以像素为单位指定。
- `srcm` 属性：类似 CBG 按钮映射。执行 `INPUT` 系扩展模式或 `INPUTMOUSEKEY` 时，鼠标下的按钮映射图片颜色（RGB 部分）会写入 `RESULT:3`（或 `INPUTMOUSEKEY` 中的 `RESULT:6`）。
